...
import styles from '@styles/components/Dashboard/DataFilterButtons/DropdownList/dropdownList.module.scss'

interface IProps {
  inView: boolean
}

type TButton = 'ROAS' | '광고비' | '노출수' | '클릭수' | '전환수' | '매출' | '없음'

const DATA_LIST: TButton[] = ['ROAS', '광고비', '노출수', '클릭수', '전환수', '매출', '없음']

const TwoDropDownList = ({ inView }: IProps) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    setCategory((prev) => ({
      ...prev,
      twoSelectCategory: koreanToCategoryDict[value as TButton] as TDashBoardCategory,
    }))
  }
...
export default TwoDropDownList
